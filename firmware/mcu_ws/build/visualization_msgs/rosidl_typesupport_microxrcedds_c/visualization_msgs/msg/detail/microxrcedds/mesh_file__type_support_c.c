...
#include "visualization_msgs/msg/detail/mesh_file__rosidl_typesupport_microxrcedds_c.h"
...
typedef visualization_msgs__msg__MeshFile _MeshFile__ros_msg_type;
  ...
  ucdrBuffer * cdr)
{
  ...
  _MeshFile__ros_msg_type * ros_message = (_MeshFile__ros_msg_type *)(untyped_ros_message);
  (void)ros_message;

  // Member: filename
 {
    uint32_t string_len = (ros_message->filename.data == NULL) ? 0 : (uint32_t)strlen(ros_message->filename.data) + 1;
    ros_message->filename.size = (ros_message->filename.data == NULL) ? 0 : string_len - 1 ;
    rv = ucdr_serialize_sequence_char(cdr, ros_message->filename.data, string_len);
  }
  // Member: data
  {
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: filename
  {
    size_t capacity = ros_message->filename.capacity;
    uint32_t string_size;
    rv = ucdr_deserialize_sequence_char(cdr, ros_message->filename.data, capacity, &string_size);
    if (rv) {
      ros_message->filename.size = (string_size == 0) ? 0 : string_size - 1;
    } else if(string_size > capacity){
      cdr->error = false;
      cdr->last_data_size = 1;
      ros_message->filename.size = 0;
  ...
  size_t current_alignment)
{
  ...
  const _MeshFile__ros_msg_type * ros_message = (const _MeshFile__ros_msg_type *)(untyped_ros_message);
  (void)ros_message;
...
{
  bool full_bounded;
  return max_serialized_size_visualization_msgs__msg__MeshFile(&full_bounded, 0);
}
